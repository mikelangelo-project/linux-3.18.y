...
static void vhost_blk_handle_guest_kick(struct vhost_work *work)
{
	...
	for (;;) {
		...
		move_iovec(vq->iov, /*vq->*/iov_hdr, sizeof(hdr), out);
		ret = memcpy_fromiovecend((unsigned char *)&hdr, /*vq->*/iov_hdr, 0,
					   sizeof(hdr));
		if (ret < 0) {
...
static int vhost_blk_open(struct inode *inode, struct file *file)
{
	...
	vqs[VHOST_BLK_VQ_REQ] = &blk->vq;

	vhost_dev_init(&blk->dev, vqs/*&blk->vq*/, VHOST_BLK_VQ_MAX);
